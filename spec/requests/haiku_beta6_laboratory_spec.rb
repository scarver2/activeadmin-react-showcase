# spec/requests/haiku_beta6_laboratory_spec.rb
# frozen_string_literal: true

require "rails_helper"

RSpec.describe "Haiku beta6 Laboratory" do
  ...
  context "with an authenticated administrator" do
    ...
    it "renders host-owned records through all 24 Haiku beta6 roles" do
      account = create(:account, name: "Haiku Sample")
      get admin_haiku_beta6_laboratory_path

      expect(response).to have_http_status(:ok)
      expect(response.body).to include("Haiku beta6 — Heritage Laboratory", "Haiku Sample", admin_account_path(account))
      expect(response.body).to include('data-activeadmin-theme="haiku_beta6"')
      expect(response.body).not_to include('data-activeadmin-theme="aros_zune"', 'data-activeadmin-theme="amigaos_4"')
      expect(response.body).to include(%(class="#{ActiveAdmin::Themes::HaikuBeta6::COMPOSITION.class_for(:workspace)}"))
      ...
      aggregate_failures "composition slots" do
        expect(ActiveAdmin::Themes::HaikuBeta6::COMPOSITION.slots.length).to eq(24)
        ActiveAdmin::Themes::HaikuBeta6::COMPOSITION.slots.each_value do |css_class|
    ...
    it "filters through a GET form without changing records" do
      ...
      expect { get admin_haiku_beta6_laboratory_path, params: { status: "trial" } }.not_to change(Account, :count)
    ...
    it "escapes records and does not reflect unsupported filters" do
      ...
      get admin_haiku_beta6_laboratory_path, params: { status: "<script>bad()</script>" }
    ...
    it "isolates Haiku beta6 from preceding heritage routes" do
      get admin_aros_zune_laboratory_path
      expect(response.body).to include('data-activeadmin-theme="aros_zune"')
      expect(response.body).not_to include('data-activeadmin-theme="haiku_beta6"')
      ...
      get admin_root_path
      expect(response.body).not_to include('data-activeadmin-theme="haiku_beta6"')
      expect(response.body).not_to include(ActiveAdmin::Themes::HaikuBeta6::COMPOSITION.class_for(:workspace))
    end
  end
end
