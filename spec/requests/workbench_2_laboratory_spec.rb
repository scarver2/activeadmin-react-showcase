# spec/requests/workbench_2_laboratory_spec.rb
# frozen_string_literal: true

require "rails_helper"

RSpec.describe "Workbench 2 Laboratory" do
  ...
  context "with an authenticated administrator" do
    ...
    it "renders accessible server-owned records through the Workbench 2 composition" do
      account = create(:account, name: "Workbench 2 Sample")
      get admin_workbench_2_laboratory_path

      expect(response).to have_http_status(:ok)
      expect(response.body).to include("Workbench 2.x — Heritage Laboratory", "Workbench 2 Sample", admin_account_path(account))
      ...
      expect(response.body).to include(%(class="#{ActiveAdmin::Themes::Workbench2::COMPOSITION.class_for(:workspace)}"))
      ...
      aggregate_failures "composition slots" do
        ActiveAdmin::Themes::Workbench2::COMPOSITION.slots.each_value do |css_class|
    ...
    it "filters through a GET form without changing records" do
      ...
      expect { get admin_workbench_2_laboratory_path, params: { status: "trial" } }.not_to change(Account, :count)
    ...
    it "escapes records and does not reflect unsupported filters" do
      ...
      get admin_workbench_2_laboratory_path, params: { status: "<script>bad()</script>" }
    ...
    it "keeps each historical composition isolated to its own route" do
      ...
      expect(response.body).not_to include('data-activeadmin-theme="workbench-2"')

      get admin_root_path
      expect(response.body).not_to include('data-activeadmin-theme="workbench-2"')
      expect(response.body).not_to include(ActiveAdmin::Themes::Workbench2::COMPOSITION.class_for(:workspace))
    end
  end
end
