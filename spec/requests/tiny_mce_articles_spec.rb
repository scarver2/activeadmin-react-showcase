# spec/requests/tiny_mce_articles_spec.rb
# frozen_string_literal: true

require "rails_helper"

RSpec.describe "TinyMCE articles" do
  ...
  it "requires an authenticated administrator" do
    ...
    get new_admin_tiny_mce_article_path
  ...
  it "renders one enhancement island over a meaningful textarea fallback" do
    ...
    expect(response.parsed_body.css('[data-react-component="TinyMceEditor"]').size).to eq(1)
    expect(response.parsed_body.at_css("textarea#tiny_mce_article_body_html")["name"]).to eq("tiny_mce_article[body_html]")
    expect(response.body).to include("Demo", "Ruby", "JavaScript", "Architecture")
    expect(response.body).to include("no cloud key", "Asset Manager")
  end

  it "creates a scoped article and persists only server-sanitized markup" do
    expect do
      post admin_tiny_mce_articles_path, params: {
        tiny_mce_article: {
          body_html: '<h2>Briefing</h2><p onclick="alert(1)">Keep me</p><script>alert(1)</script>',
          summary: "Safe preview",
          title: "Sanitized TinyMCE article"
        }
      }
    end.to change(admin.tiny_mce_articles, :count).by(1)

    article = admin.tiny_mce_articles.last
    expect(response).to redirect_to(admin_tiny_mce_article_path(article))
  ...
  it "preserves submitted editor content when Rails validation fails" do
    ...
    expect(TinyMceArticle.find_by(summary: "Validation proof")).to be_nil
  end

  it "prevents administrators from reading or changing another administrator's article" do
    other_article = create(:tiny_mce_article)

    get admin_tiny_mce_article_path(other_article)
    expect(response).to have_http_status(:not_found)

    sign_in admin
    patch admin_tiny_mce_article_path(other_article), params: {
      tiny_mce_article: { body_html: "<p>Unauthorized</p>", title: "Unauthorized" }
    }
    expect(response).to have_http_status(:not_found)
  ...
  it "returns a conflict without losing a stale editor draft" do
    article = create(:tiny_mce_article, admin_user: admin)
    stale_version = article.lock_version
    article.update!(summary: "Changed elsewhere")

    patch admin_tiny_mce_article_path(article), params: {
      tiny_mce_article: {
