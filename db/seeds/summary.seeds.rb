...
after :accounts, :activity_center, :audit_history, :calendar, :content, :geospatial, :hierarchy, :operator_chat, :workflow do
  ...
  puts "Seeded #{Account.count} accounts, #{Contact.count} contacts, #{DailyMetric.count} daily metrics, " \
       ...
       "#{ShowcaseAsset.count} assets, #{ScheduleEvent.count} calendar events, #{HierarchyNode.count} hierarchy nodes, " \
