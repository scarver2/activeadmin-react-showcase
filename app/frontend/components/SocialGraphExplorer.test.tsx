...
const people = [{ id: "1", name: "Luke Skywalker" }, { id: "2", name: "Darth Vader" }, { id: "3", name: "Leia Organa" }]
const node = (id: string, name: string, degree: number | null = 0) => ({ degree, headline: `${name} role`, id, name, url: `/people/${id}` })
const graph = { edges: [{ id: "1-2", label: "father and son", source: "1", target: "2" }], mutuals: [node("3", "Leia Organa", null)], nodes: [node("1", "Luke Skywalker"), node("2", "Darth Vader", 1)], path: ["1", "2"] }
...
describe("SocialGraphExplorer", () => {
  it("creates, highlights, selects, fits, changes depth/root/target, and disposes", async () => {
    ...
    expect(instance.addClass).toHaveBeenCalledTimes(3)
    act(() => instance.handler({ target: { id: () => "2" } }))
    expect(screen.getByRole("region", { name: "Selected person" })).toHaveTextContent("Darth Vader role")
    expect(screen.getByRole("region", { name: "Relationship path" })).toHaveTextContent("Luke Skywalker — father and son → Darth Vader")
    await userEvent.click(screen.getByRole("button", { name: "Fit graph" }))
    expect(instance.fit).toHaveBeenCalled()
    await userEvent.click(screen.getByRole("button", { name: "Luke Skywalker" }))
  ...
  it("uses an id when a path node is absent from the people projection", () => {
    render(<SocialGraphExplorer endpoint="/graph" graph={{ ...graph, path: ["1", "missing"] }} people={people} rootId="1" />)
    expect(screen.getByText("Luke Skywalker — connected → missing")).toBeVisible()
  })
