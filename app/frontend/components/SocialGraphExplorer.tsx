...
type Edge = { id: string, label: string, source: string, target: string }
...
export default function SocialGraphExplorer({ endpoint, graph: initialGraph, people, rootId: initialRoot }: SocialGraphExplorerProps) {
  ...
  useEffect(() => {
    ...
    ], layout: { name: "breadthfirst", animate: false, directed: true, roots: [ rootId ], spacingFactor: 1.25 }, style: [
      { selector: "node", style: { "background-color": "#0f766e", label: "data(label)", "font-size": 11, "text-background-color": "#ffffff", "text-background-opacity": 0.85, "text-background-padding": "2px" } },
      { selector: "edge", style: { "font-size": 8, "line-color": "#94a3b8", label: "data(label)", "text-background-color": "#ffffff", "text-background-opacity": 0.85, "text-background-padding": "2px", "text-rotation": "autorotate", width: 2 } },
      { selector: ".path", style: { "background-color": "#b45309", "line-color": "#b45309", "target-arrow-color": "#b45309", width: 4 } }
    ] })
    instance.on("tap", "node", (event) => setSelectedId(event.target.id()))
    graph.path.forEach((id) => instance.getElementById(id).addClass("path"))
    graph.path.slice(1).forEach((id, index) => {
      const pair = [ graph.path[index], id ].sort((left, right) => Number(left) - Number(right))
      instance.getElementById(pair.join("-")).addClass("path")
    })
    cy.current = instance
...
function relationshipPath(graph: Graph, people: Pick<Person, "id" | "name">[]) {
  return graph.path.map((id, index) => {
    const name = people.find((person) => person.id === id)?.name || id
    const nextId = graph.path[index + 1]
    if (!nextId) return name
    const edge = graph.edges.find(({ source, target }) => [ source, target ].includes(id) && [ source, target ].includes(nextId))
    return `${name} — ${edge?.label || "connected"} → `
  }).join("")
}
