// test/browser/workbench_2_laboratory.spec.ts
...
for (const width of [1440, 390]) {
  test(`Workbench 2.x preserves AA4 behavior at ${width}px`, async ({ browser }) => {
    const context = await browser.newContext({ viewport: { width, height: 1000 } })
    const page = await context.newPage()
    await signIn(page)
    await page.goto("/admin/workbench_2_laboratory")

    await expect(page).toHaveTitle(/Workbench 2.x — Heritage Laboratory/)
    await expect(page.locator("body")).toHaveAttribute("data-activeadmin-theme", "workbench-2")
    await expect(page.locator('[data-activeadmin-theme="workbench-13"]')).toHaveCount(0)
    const study = page.getByRole("region", { name: "Workbench 2.x design study", exact: true })
    await expect(study).toBeVisible()
    await expect(study).toHaveCSS("background-color", "rgb(155, 155, 155)")
    await expect(study.getByRole("button", { name: "Use", exact: true })).toHaveCSS("background-color", "rgb(49, 95, 155)")
    await expect(study.getByRole("button", { name: "Use", exact: true })).toHaveCSS("color", "rgb(255, 255, 255)")
    await expect(study.getByRole("link", { name: "Reset", exact: true })).toHaveCSS("color", "rgb(17, 17, 17)")
    ...
    if (process.env.CAPTURE_SHOWCASE_SCREENSHOTS) {
      await page.screenshot({ fullPage: true, path: `docs/screenshots/workbench-2-${width}.png` })
    }
    ...
    await expect(study.getByText(/documented Release 2 pen roles/)).toBeVisible()
    ...
    await expect(page.locator('[data-activeadmin-theme="workbench-2"]')).toHaveCount(0)
    await context.close()
  })
}

test("Workbench 2.x form and links remain useful without JavaScript", async ({ browser }) => {
  ...
  await expect(page).toHaveURL(/status=trial/)
  const statusTags = page.getByRole("region", { name: "Workbench 2.x design study", exact: true })
    .locator("tbody td:last-child span")
  await expect(statusTags).not.toHaveCount(0)
  expect((await statusTags.allTextContents()).every(status => status === "trial")).toBe(true)
  await page.getByRole("region", { name: "Workbench 2.x design study", exact: true })
