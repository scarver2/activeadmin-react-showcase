...
for (const width of [1440, 390]) {
  test(`AROS Zune preserves AA4 behavior and fixed presentation at ${width}px`, async ({ browser }) => {
    const context = await browser.newContext({ viewport: { width, height: 1000 } })
    const page = await context.newPage()
    await signIn(page)
    await page.goto("/admin/aros_zune_laboratory")

    await expect(page).toHaveTitle(/AROS\/Zune — Heritage Laboratory/)
    await expect(page.locator("body")).toHaveAttribute("data-activeadmin-theme", "aros_zune")
    ...
    const study = page.getByRole("region", { name: "AROS Zune design study", exact: true })
    await expect(study).toBeVisible()
    await expect(study).toHaveCSS("color", "rgb(32, 41, 51)")
    await expect(study).toHaveCSS("--aros-zune-active", "#4b72a5")
    await expect(study.locator(".aros-zune-primary-window > .aros-zune-window-header")).toHaveCSS("color", "rgb(255, 255, 255)")
    ...
    if (process.env.CAPTURE_SHOWCASE_SCREENSHOTS) {
      await page.screenshot({ fullPage: true, path: `docs/screenshots/aros-zune-${width}.png` })
    }
...
test("AROS Zune form and record links remain useful without JavaScript", async ({ browser }) => {
