...
for (const width of [1440, 390]) {
  test(`AmigaOS 4 preserves AA4 behavior and fixed presentation at ${width}px`, async ({ browser }) => {
    const context = await browser.newContext({ viewport: { width, height: 1000 } })
    const page = await context.newPage()
    await signIn(page)
    await page.goto("/admin/amigaos_4_laboratory")

    await expect(page).toHaveTitle(/AmigaOS 4 — Heritage Laboratory/)
    await expect(page.locator("body")).toHaveAttribute("data-activeadmin-theme", "amigaos_4")
    ...
    const study = page.getByRole("region", { name: "AmigaOS 4 design study", exact: true })
    await expect(study).toBeVisible()
    await expect(study).toHaveCSS("color", "rgb(23, 38, 50)")
    await expect(study).toHaveCSS("--amigaos-4-active", "#197ac1")
    await expect(study.locator(".amigaos-4-primary-window > .amigaos-4-window-header")).toHaveCSS("color", "rgb(255, 255, 255)")
    ...
    if (process.env.CAPTURE_SHOWCASE_SCREENSHOTS) {
      await page.screenshot({ fullPage: true, path: `docs/screenshots/amigaos-4-${width}.png` })
    }
...
test("AmigaOS 4 form and record links remain useful without JavaScript", async ({ browser }) => {
